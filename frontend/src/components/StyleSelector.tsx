...
const StyleSelector = ({value, onChange, isOpen, setIsOpen} : {value: ThumbnailStyle; onChange: (style: ThumbnailStyle) => void; isOpen: boolean; setIsOpen: (open: boolean) => void}) => {
    const styleDescriptions: Record<ThumbnailStyle, string> = {
        'Bold & Graphic': 'Hight contrast, bold typography, striking visuals',
    ...
    const styleIcons : Record<ThumbnailStyle, React.ReactNode> = {
        ...
        'Photorealistic': <SparkleIcon className="h-4 w-4" />,
        "Illustrated": <SparkleIcon className="h-4 w-4" />,
            ...
            {thumbnailStyles.map((style) => (
                ...
                >
                    ...
                    <div>
                        ...
                        <p className="text-xs te">{styleDescriptions[style]}</p>
                    </div>
                </button>
            ))}
