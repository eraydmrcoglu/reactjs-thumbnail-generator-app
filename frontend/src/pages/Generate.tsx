...
const Generate = () => {
  ...
  const fetchThumbnail = async()=>{
    try {
      ...
      setLoading(!data?.thumbnail?.imageUrl);
      setAdditionalDetails(data?.thumbnail?.user_prompt);
      setTitle(data?.thumbnail?.title);
      setAspectRatio(data?.thumbnail?.aspectRatio);
      setColorSchemeId(data?.thumbnail?.colorScheme);
          ...
          <div className="grid lg:grid-cols-[400px_1fr] gap-8">
              ...
              <div className="p-6 rounded-2xl bg-white/8 border border-white/12 shadow-xl space-y-6">
                ...
                <div className="space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea value={additionalDetails} onChange={(e)=>setAdditionalDetails(e.target.value)} rows={3} placeholder='Add any specific elements, mood, or style preferences..' className='w-full py-3 px-4 rounded-lg border border-white/10 bg-white/6 text-zinc-100 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-pink-500 resize-none' />
                  </div>
                </div>
            ...
            <div className="">
              <div className="p-6 rounded-2xl bg-white/8 border border-white/10 shadow-xl">
                ...
                <PreviewPanel isLoading={loading} thumbnail={thumbnail} aspectRatio={aspectRatio} />
              </div>
            </div>
          </div>
        </main>
